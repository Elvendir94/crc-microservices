...
internal class MicroserviceAResponseConsumer(
    ...
    fun process():
            Consumer<Flux<Message<Map<String, Any?>>>> = Consumer { input ->
        input
            .map { message ->
                EventDTO(
                    ...
                    .map {
                        ...
                        DomainObject(objectToCorrect.messageId, eventDto.eventBody.fieldA, eventDto.eventBody.fieldB)
                                    ...
                                    domainObjectRepository.save(domainObject)
                                            .doOnNext { saved ->
                                                logger.info { "✅ DomainObject saved: $saved" }
                                            }
                                }.flatMap { _ ->
                                    stateStoreRepository.save(eventDto)
                                            .doOnNext {
                                                logger.info { "🗃️ EventDTO saved to stateStoreRepository: $it" }
                                            }
                                }
            }
            .doOnNext {
                logger.info { "DomainObject saved" }
            }.subscribe()
    }
